...
-   **What Changed History (7-30 Days)**: LocalStorage-based daily snapshot persistence (max 45 snapshots) tracking market conditions over time. Features: (1) Auto-capture on Daily Brief page load, (2) Manual save option in footer, (3) Dedicated History page (`/history`) with timeline view, 7D/30D filters, and detail panels, (4) Delta comparison engine detecting regime shifts, Fed tone changes, policy risk changes, volatility state changes, and trade condition level changes, (5) "What Changed" section in Daily Brief uses snapshot comparison with API fallback, (6) History section in Settings with export/clear functionality.
-   **Evidence Mode**: Transparency feature allowing users to toggle visibility of data sources, freshness indicators, AI input context, and provenance information across multiple pages. Features: (1) Toggle button (EvidenceToggle) with localStorage persistence (key: `evidence_mode_v1`), (2) Collapsible EvidencePanel components showing source information, (3) SourceChip badges for inline source attribution, (4) Integration across Daily Brief (market call, what changed, headlines), Policy (clusters, Fed quotes), AI Insights (prompt context with copy-to-clipboard), and News Stream (headline source chips, matched symbols), (5) Accessible with aria-pressed and aria-label attributes.
